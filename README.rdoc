...
If you use MIDI alot, you'll notice that there was no mention of MIDI channel or a velocity value for the notes sent in the last example.

That is because channel and velocity have sticky values (and default to 0 and 100, respectively).  These sticky values will be used by any commands where they are needed.  If one passes a channel or velocity value explicitly to a command, the sticky value will be overriden for that command.
