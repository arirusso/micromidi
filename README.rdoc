...
* MIDI Thru, MIDI processing and custom input events
...
Here are some basic examples that use Unimidi inputs and outputs.  ({read more here about selecting an output...}[http://github.com/arirusso/unimidi/blob/master/examples/select_a_device.rb])
...
While this next example is running, all input is sent directly to the output except for notes; notes that are received are only sent to the output if their octave is between 1 and 3.  Output is also printed to the console by passing in <em>$stdout</em>.
...
I've written up a few posts explaining how these examples work in greater detail
...
* {Shorthand aliases for... everything}[http://tx81z.blogspot.com/2011/08/micromidi-shorthand.html]
