...
While this following example is running, all input is sent to an output except for notes.  Those received notes are only sent to the output if their octave is between 1 and 3.  Output is also printed to the console by passing in <em>$stdout</em>.
