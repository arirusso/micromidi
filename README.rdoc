...
This example sends all inputted messages to an output, except for note messages which are transposed up an octave first.  All output is printed to the console as well
  ...
  MIDI.message($stdout, @i, @o) do

    thru

    receive :note do |message|
      output transpose(message, :octave, 1)
    end
...
I've written up a few posts explaining each of micromidi's features
